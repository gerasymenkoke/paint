...
class DrawPencil @JvmOverloads constructor(
    ...
    override fun onTouchEvent(event1: MotionEvent): Boolean {
        ...
        if (j >=1 && j<=9)
             {   rx=(((x-x1)* 100.0).roundToInt() / 100.0).toFloat() / x
                 ry=(((y-y1)* 100.0).roundToInt() / 100.0).toFloat() / y
     ...
     rx=(((x-0)* 100.0).roundToInt() / 100.0).toFloat() / x
     ry=(((y-0)* 100.0).roundToInt() / 100.0).toFloat() / y
        crx_[j] = rx
        cry_[j] = ry
        x1=x
        y1=y
