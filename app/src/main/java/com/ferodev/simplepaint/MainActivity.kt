...
while ( j <= cin  )  {
if    ( cin<=0 ||  ( signx_[j]==2.0f && signy_[j]==2.0f) ||  ( signx_[j] == signx_[kk] && signy_[j] == signy_[kk]  && (Math.abs( (cangle_[j]) - (cangle_[kk]) ) >= 0)) || ( (signx_[j] != signx_[kk] || signy_[j] != signy_[kk] ) && (Math.abs( (cangle_[j]) - (cangle_[kk]) ) >= 0) )  )
